...
// migration is enabled, so it deliberately ignores all current v2 SVG poses.
  // P-flat.0: live idle/sleep coat interiors resolve to the flat owning
  // baseColor / regionColors.* value. Outlines keep silhouette contrast;
  // ear-inner, nose, eyes, lids, and markings stay independently owned.
  // Legacy source tints are no longer shifted into custom coats as stray
  // pale/grey fragments.
(() => {
  ...
  function applyPalette(svg, pattern = {}) {
    ...
    // V6-M0: palette recolours V4 SVG pixels only. It must never own or mutate
    // document.body.dataset.catcodeModel (that reset silently forced black V4
    // over an already-latched v6-idle-preview).
    ...
    // Preserve exact configured nose including #000000 (truthy string).
    const earInner =
      typeof pattern.earInnerColor === "string" && pattern.earInnerColor.trim()
        ? pattern.earInnerColor.trim()
        : "#f18aa5";
    const nose =
      typeof pattern.noseColor === "string" && pattern.noseColor.trim()
        ? pattern.noseColor.trim()
        : "#304055";
    ...
    function applyForcedSemantic(pixel, coat) {
      let next = null;
      if (pixel.classList.contains("v4-eye-outline")) next = eyeOutline;
      else if (pixel.classList.contains("v4-ear-inner")) next = earInner;
      else if (pixel.classList.contains("v4-nose")) next = nose;
      else if (pixel.classList.contains("v4-closed-lid")) next = lidColorForCoat(coat);
      else if (pixel.classList.contains("v4-iris-left")) {
        next = eyeStyle === "solid" ? solidFillLeft : irisLeft;
      } else if (pixel.classList.contains("v4-iris-right")) {
        ...
        next = eyePupil;
      } else if (pixel.classList.contains("v4-eye-bg-left")) {
        next = eyeStyle === "solid" ? solidFillLeft : eyeBg;
      } else if (pixel.classList.contains("v4-eye-bg-right")) {
    ...
    svg.querySelectorAll(".v4-pixel").forEach((pixel) => {
      ...
      const coat = coatForPixel(pixel);
      const coatRgb = parseHex(coat) || parseHex(base) || [32, 36, 45];

      // P-flat.0: semantic roles must paint even if a cell lacks --v4-source-*.
      if (!match) {
        applyForcedSemantic(pixel, coat);
        return;
      }
      ...
      const regionOverride =
        ...
        regionColors[regionKey]
          ? regionColors[regionKey]
          : "";
      // P-flat.0: coat/region interiors are the flat owner colour — no legacy
      // source-light / source-shadow hue leakage into custom coats.
      let next = coat;
      let forceDirect = !!regionOverride;

      if (pixel.classList.contains("v4-outline")) {
        // Silhouette outline only: keep dark edge; do not flatten to coat fill.
        next =
          luminance(coatRgb) > 150
            ? "#1c1c1c"
            : coatColorForSource(source, coat);
        forceDirect = true;
      }
      ...
      if (
        ...
        // S-ear.0.1: outer ear shell is flat ears colour; outline stays edge.
        if (pixel.classList.contains("v4-outline")) {
      ...
      if (applyForcedSemantic(pixel, coat)) {
        return;
      }

      // Region overrides must not share CSS variables with other regions.
      if (forceDirect) {
        setPixelFill(pixel, next);
      } else {
