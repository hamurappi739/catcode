  ...
  var Ut = V((Oa, Ot) => {
    ...
    function Br({
      ...
      function ae(x) {
        ...
        if ((((t.dataset.catcodeModel === "v6-idle-preview" ? g(x.clientX, x.clientY) : m(x.clientX, x.clientY))
          ? a(x.clientX, x.clientY)
          : F || s(p)), !h))
          return;
        if (!(x.buttons & 1)) {
          ie(x);
...
`)),
  ...
  var Na = V(() => {
    ...
    function showIgnoredStretchReaction() {
      (ke(), De(), Ge(),
        ...
        (healthAngryTimer = setTimeout(() => {
          ((healthAngryTimer = null), clearHealthAngryGrowls(), setHealthAngry(!1));
        }, 12e3)));
    }
    let v4Pattern = null;
    ...
    function ha(e, t) {
      ...
      // V6 purr is intentionally head-only. The cat's PNG has no semantic
      // regions, so use the authored head band of the active full-frame pose.
      // This preserves every painted head pixel, including ears and cheeks,
      // while rejecting torso, paws, tail, and the transparent area below it.
